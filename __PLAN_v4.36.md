...
| **v4.36** | 2026.04.04 | 이벤트 로그 개선 — 개발자 기기 수집 제외(dc_is_dev 플래그), 유입 경로 수집(referrer, current_url, utm) |
...
| 2026.04.04 | v4.36 | 이벤트 로그 개선 — `dc_is_dev` localStorage 플래그로 개발자 기기 수집 제외. `logEvent()`에 유입 경로 수집 추가(referrer, current_url, utm_source, utm_medium, utm_campaign). `event_logs` 테이블 컬럼 5개 추가 |