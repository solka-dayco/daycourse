...
## v4.2 업데이트 계획 (Chat GPT에서 작성한 계획, Claude에 맞춰서 재작성 및 재구성 필요)

### R1. 코스 시간 입력 UX 개선

#### 문제 정의

* 코스 제작 시 이동시간/체류시간 입력이 필수로 요구됨
* 이동 수단(도보, 대중교통, 자가용)에 따라 시간 편차가 커 정확한 입력이 어려움
* 입력 피로도로 인해 코스 제작 이탈 발생

#### 개선 방향

* 이동시간/체류시간 입력을 필수값에서 제거
* 총 소요시간 중심 구조로 변경

#### 상세 설계

* 총 소요시간 입력 필수화
* 이동시간/체류시간 입력은 선택 기능으로 유지
* 선택 UI:

  * 저장 버튼 근처에 “세부사항 입력” 텍스트만 노출
  * 별도 강조 없음

#### 유지 기능

* 코스 간 거리 자동 측정 기능은 그대로 유지
...
* 기존 “인용하기” 기능을 “계획 담기”로 변경
* 피드의 코스를 자신의 계획으로 저장

#### 계획 → 경험 전환

* 기존 계획 데이터를 기반으로 create 페이지 진입
* 수정 후 업로드 가능
...
* 임시저장 및 상태 전환 시 데이터 꼬임 발생 가능성 있음
* 경로 이동(뒤로가기, 새로고침 등)에 따른 상태 관리 주의
